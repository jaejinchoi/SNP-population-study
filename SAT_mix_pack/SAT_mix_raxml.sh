...
print_help_and_exit () {
    ...
    echo -e "Adjusted functions(reqiure installation)"
    echo -e "1. Admixture"
    echo -e "2. TreeMix"
    echo 1>&2
    ...
    Parameters '*' at the end are necessary to TURN-ON its function
    \n----------ETC
    [-h, help]" 1>&2
	echo
    echo 1>&2
    echo -e "Options:" 1>&2
    echo -e "\t-b: Performs (non-parametric) bootstrap analysis and generate a tree" 1>&2
...
if [ ! -z "${vcf_file}" ]
then
    "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -v "${mnn_filtered_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" #hapmap
    [ $? != 0 ] && exit 1
   
elif [ ! -z "${hap_file}" ]
then
    "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -H "${mnn_filtered_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" #VCF
    [ $? != 0 ] && exit 1

elif [ ! -z "${gds_file}" ]
then
    "${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -d "${mnn_filtered_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}" #GDS
    [ $? != 0 ] && exit 1
...
if [ ${num_bs_sample} -ge 100 ] #since Bs minimum is 100
then
	...
	#SNP sequence alignment using MUSCLE, takes much of time, should consider accuracy depend on argument usage
        ...
        if [ ${seq_len} -ne 0 ]; then #clustal omega alignment, output phylip format --outfmt=phy
            echo "clustalo --threads=${n_thread} -t DNA -i ${prefix_output}.fasta -o ${snphylo_prefix}.phylip.txt --outfmt=phy"
            clustalo --threads=${n_thread} -t DNA -i ${prefix_output}.fasta -o ${snphylo_prefix}.phylip.txt --outfmt=phy; [ $? != 0 ] && exit 1

        #using MUSCLE, using single core output as phylip format -phyi
        elif [ ${seq_len} -lt 50000 ]; then #normal alignment
	    echo "${MUSCLE} -phyi  -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -quiet"
	    "${MUSCLE}" -phyi  -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -quiet; [ $? != 0 ] && exit 1

	elif [ ${seq_len} -lt 100000 ]; then #large
	    echo "${MUSCLE} -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 2 -quiet"
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 2 -quiet; [ $? != 0 ] && exit 1
	    
	elif [ ${seq_len} -ge 100000 ]; then #huge, -ge greater or equal
	    echo "${MUSCLE} -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 1 -diags -sv -quiet"
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 1 -diags -sv -quiet; [ $? != 0 ] && exit 1
	...
	then
	    ...
	    #raxmlHPC-PTHREADS-AVX -T 6 -s ./infile -n ./outfile -m GTRCAT -f a; [ $? != 0 ] && exit 1 #using raxMLHPC
            ...
            echo "/home/jjc/download/raxml-ng_v1.0.1/raxml-ng --threads ${n_thread} --msa ${snphylo_prefix}.phylip.txt  --msa-format PHYLIP --prefix ${snphylo_prefix}.out --model GTR+G --all --bs-trees ${num_bs_sample}"
            /home/jjc/download/raxml-ng_v1.0.1/raxml-ng --threads ${n_thread} --msa ${snphylo_prefix}.phylip.txt --msa-format PHYLIP --prefix ${snphylo_prefix}.out --model GTR+G --all --bs-trees ${num_bs_sample} > /dev/null; [ $? != 0 ] && exit 1

            echo "# run-log saved in ${snphylo_prefix}.raxml.log"

	    #echo "2\n | y\n | phylip dnaml" #what is using
	    #echo -e "2\n" | "y\n" | phylip dnaml; [ $? != 0 ] && exit 1 #interactive call. call dnaml in phylip; 2\n (off print progress)
	else
	    out_sample_no=$[$(grep -ne "\<${out_sample_id}\>" infile | cut -f1 -d":") - 1]
	    
            if [ ${out_sample_no} -eq -1 ]
	    then
	        echo "Error!!! There is no sample name (${out_sample_id}) to use as a outgroup in a tree input file (${snphylo_prefix}.phylip.txt)." 1>&2
	...
	#"${R}" --slave --vanilla --file="${SCRIPTS_DIR}/determine_bs_tree.R" --args -i "${snphylo_prefix}.ml.tree" -o "${snphylo_prefix}" -p "${snphylo_prefix}.phylip.txt" -n ${num_bs_sample} > /dev/null
	#[ $? != 0 ] && exit 1
