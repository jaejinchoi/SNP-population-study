...
while getopts "v:H:d:a:c:p:l:k:K:t:r:R:m:M:P:o:s:b:h" OPT
do
    case "${OPT}" in
        'v')
            vcf_file="${OPTARG}"
...
"${R}" --slave --vanilla --file="${SCRIPTS_DIR}/generate_snp_sequence.R" --args -H "${mnn_filtered_file}" -l "${ld_threshold}" -m "${maf_threshold}" -M "${missing_rate}" -o "${prefix_output}"
[ $? != 0 ] && exit 1
...
then
	...
	for ((k=2;k<8;k++)) #2 to 7
		do
		    #"${SCRIPTS_DIR}/admixture" --cv "${prefix_output}_12" $k, admixture require 1,2 based ped
		    ${ADMIXTURE} --cv "${admixture_prefix}_12.ped" ${k} > "${admixture_prefix}_12.${k}.log"
		    [ $? != 0 ] && exit 1
...
then
	...
	if [ ${seq_len} -lt 50000 ]; then #normal alignment
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt; [ $? != 0 ] && exit 1

	elif [ ${seq_len} -lt 100000 ]; then #large
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 2; [ $? != 0 ] && exit 1
	    
	elif [ ${seq_len} -ge 100000 ]; then #huge, -ge greater or equal
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.phylip.txt -maxiters 1 -diags -sv; [ $? != 0 ] && exit 1
	...
	ln -sf ${snphylo_prefix}.phylip.txt infile
