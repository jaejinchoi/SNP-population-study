...
#2014.5.16 print index number of selected SNP, is this necessary?
#snp_id.file <- sprintf("%s.snp_index", file.prefix)
#write.table(snpset.id, file=snp_id.file, sep=' ', row.names=F, col.names=F, quote=F) #2014.5.15 save snp

#no gds to hapmap available at the time. Instead, convert gds to ped, checked.
gds2.ped <- sprintf("%s.picked", file.prefix) #create .ped and .map
snpgdsGDS2PED(genofile, ped.fn=gds2.ped, sample.id=NULL, snp.id=snpset.id, use.snp.rsid=TRUE,
              format=c("A/G/C/T", "A/B", "1/2"), verbose=F) #SNP phylogeny use ACGT forma, sample.id=NULL (all sample used)
