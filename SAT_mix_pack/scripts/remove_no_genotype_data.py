...
for snp_line in open(hapmap_file, "r"):
    ...
    if (snp_line.strip()==""):
        ...
        #max_num_nss = (len(snp_line.strip().split()) - 11) * (max_nss_percent / 100.0) #original
        ...
        try: #wonder why this is not empty string tolerate
            _ = int(snp_data[2])
        except ValueError:
            num_wrong_chr_id += 1
            continue
        ...
        #pass-if missing is over threshold or monoallelic
        if (genotype_data.count('NN') < max(1, max_num_nss)): #since max_num_nss can be 0
            sys.stdout.write(snp_line)
