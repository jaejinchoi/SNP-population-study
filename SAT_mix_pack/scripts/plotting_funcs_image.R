...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
	plot(d$x, d$y, axes = F, ylab = "", xlab = "Drift parameter", xlim = c(xmin, max(d$x)+plus), cex.lab=cex, cex.axis=cex, pch = "0") #pch = node shape, cex.lab=label size
	axis(1) #1=x 2=y
	mw = max(e[e[,5]=="MIG",4])
	...
	for(i in 1:nrow(e)){
		...
		v2 = d[d[,1] == e[i,2],]
		if (e[i,5] == "MIG"){
      #plot migration arrow
			if (plotmig){
			arrows( v1[1,]$x, v1[1,]$y, v2[1,]$x, v2[1,]$y, col = col, length = arrow, lwd=2) #col= color, ity=line type, lwd=line width, code=1 (v1<-) | 2(v2<-) arrow head pointing
			}
		}
		else{
  ...
  #font size of description = cex
	text( 0, ybar - 0.04, lab = "10 s.e.", adj = 0, cex  = cex)
	...
	lines( c(mse*10, mse*10), c(ybar- 0.01, ybar+ 0.01))
        ...
        if (mbar){
                ...
                text(xma+disp, ymi, lab = "0", adj = 0, cex = cex)
		if ( mw >0.5){ text(xma+disp, yma, lab = "1", adj = 0, cex = cex)}
                else{
			text(xma+disp, yma, lab = "0.5", adj = 0, cex = cex)
		}
		text(0, yma+0.06, lab = "Migration", adj = 0 , cex = cex)
		text(0, yma+0.03, lab = "weight", adj = 0 , cex = cex)
        }	
}
...
for (item in args)
{
  ...
  scale=T
  plotmig=T
  arrow=0.2 #?length of arrow head
  lwd=1.3 #line width
  xmin=0
  plus=0.01 #scal width
  disp=0.0004 #distance between leaf(point) and label
  ybar=0.1 #migration weight plot height

  image_save_path <- paste('', item, '.png', sep='')
  
  image_width=2000
  image_height=image_width*0.4
  #by observation, optimal image scal is width:height = 5000:1
  
  png(filename=image_save_path, width=image_width, height=image_height) #fix scale?

  plot_tree(stem=item, o = NA, cex = cex, disp = disp, plus = plus, flip = vector(), arrow = arrow, scale = T, ybar = 0.1, mbar = T, plotmig = T, plotnames = T, xmin = 0, lwd = lwd, font = font)
  #plot_tree(stem=item, o = NA, cex = 1, disp = 0.003, plus = 0.01, flip = vector(), arrow = 0.05, scale = T, ybar = 0.1, mbar = T, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1)
