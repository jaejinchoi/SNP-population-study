...
then
	...
	#SNP sequence alignment using MUSCLE, takes much of time, should consider accuracy depend on argument usage
        ...
        if [ ${seq_len} -ne 0 ]; then #clustal omega alignment, output phylip format --outfmt=phy
            #echo "clustalo --threads=${n_thread} -t DNA -i ${prefix_output}.fasta -o ${snphylo_prefix}.align.txt --outfmt=fasta"
            #clustalo --threads=${n_thread} -t DNA -i ${prefix_output}.fasta -o ${snphylo_prefix}.align.txt --outfmt=fasta; [ $? != 0 ] && exit 1
	    ...
	    echo "${MUSCLE} -phyi  -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -quiet"
	    "${MUSCLE}" -phyi  -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -quiet; [ $? != 0 ] && exit 1

	elif [ ${seq_len} -lt 100000 ]; then #large
	    echo "${MUSCLE} -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -maxiters 2 -quiet"
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -maxiters 2 -quiet; [ $? != 0 ] && exit 1
	    
	elif [ ${seq_len} -ge 100000 ]; then #huge, -ge greater or equal
	    echo "${MUSCLE} -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -maxiters 1 -diags -sv -quiet"
	    "${MUSCLE}" -phyi -in ${prefix_output}.fasta -out ${snphylo_prefix}.align.txt -maxiters 1 -diags -sv -quiet; [ $? != 0 ] && exit 1
	...
	then
	    ...
	    #raxmlHPC-PTHREADS-AVX -T 6 -s ./infile -n ./outfile -m GTRCAT -f a; [ $? != 0 ] && exit 1 #using raxMLHPC
            ...
            echo "/home/jjc/download/raxml-ng_v1.0.1/raxml-ng --threads ${n_thread} --msa ${snphylo_prefix}.align.txt  --msa-format FASTA --prefix ${snphylo_prefix} --model GTR+G --all --bs-trees ${num_bs_sample}"
            /home/jjc/download/raxml-ng_v1.0.1/raxml-ng --threads ${n_thread} --msa ${snphylo_prefix}.align.txt --msa-format FASTA --prefix ${snphylo_prefix} --model GTR+G --all --bs-trees ${num_bs_sample} > /dev/null; [ $? != 0 ] && exit 1
